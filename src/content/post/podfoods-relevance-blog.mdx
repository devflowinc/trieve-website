...
Keyword search is ideal when you exactly what you're looking for, but the wrong fit for exploring. Pod Foods seemed like an ideal user-experience to us, because their explicit business goal is **to help retailers find new independent brands and help those brands be found**.
...
Search results in Pod Foods are currently show a product's first image instead of the most similar variant. However, real queries may focus on a particular color, flavor, size, etc. We show queries constructed to retrieve a specific product variant in this section.
...
<QueryDiscussion query="raw honey bars">
  Rating of current results. Broadly, they are thrown off by the "honey" term to not focus on bars:
